"""
DINOv3 Image Encoder Module using HuggingFace Transformers.

This module implements a configurable DINOv3-based image encoder for feature extraction.
It supports:
- Multiple DINOv3 variants with pretrained weights
- Training from scratch with custom configuration
- Custom output dimensions via projection layer
- Optional dropout for regularization
- Token splitting (CLS, register tokens, patch tokens)
...
class DINOv3Encoder(nn.Module):
    """
    DINOv3-based image encoder using HuggingFace transformers.

    Args:
        variant (str): DINOv3 variant to use.
        ...
        pooling_type (str): Pooling method: 'cls', 'register', 'mean', or 'max'.
        ...
        num_register_tokens (Optional[int]): Number of register tokens. Overrides variant.
    ...
    def __init__(
        ...
        valid_pooling = ['cls', 'register', 'mean', 'max']
        ...
        if not self.pretrained and self.variant not in self._VARIANT_CONFIGS:
            if any(val is None for val in [
                ...
                raise ValueError(
                    "Unknown DINOv3 variant for scratch initialization. "
    ...
    def _get_pretrained_name(self) -> str:
        ...
        raise ValueError(
            f"Invalid DINOv3 variant '{self.variant}'. "
    ...
    def _create_from_scratch(self) -> nn.Module:
        """
        Create DINOv3 model from scratch with random initialization.
        """
        ...
        num_register_tokens = self.num_register_tokens or defaults.get('num_register_tokens', 0)
        ...
        if missing:
            raise ValueError(
                f"Missing configuration for scratch DINOv3: {missing}. "
                "Provide these parameters explicitly."
            )

        config = None
        dinov3_config_cls = getattr(transformers, 'Dinov3Config', None)
        if dinov3_config_cls is not None:
            config = dinov3_config_cls(
                image_size=image_size,
                patch_size=patch_size,
                num_channels=3,
                hidden_size=hidden_size,
            ...
            if num_register_tokens:
                setattr(config, 'num_register_tokens', num_register_tokens)
                warnings.warn(
                    "Dinov3Config is not available; using ViTConfig without "
                    "native register token support."
                )
    ...
    def _sync_model_config(self) -> None:
        ...
        self.num_register_tokens = getattr(config, 'num_register_tokens', self.num_register_tokens or 0)
        self.use_bias = getattr(config, 'qkv_bias', self.use_bias)

    def _get_feature_dim(self) -> int:
        """
        Get the feature dimension of the DINOv3 model.
        """
    ...
    def unfreeze_last_n_layers(self, n: int) -> None:
        ...
        if not hasattr(self.model, 'encoder') or not hasattr(self.model.encoder, 'layer'):
            raise AttributeError("DINOv3 model does not expose encoder layers to unfreeze.")
    ...
    def _infer_patch_grid(self, x: torch.Tensor, patch_tokens: torch.Tensor) -> Optional[Tuple[int, int]]:
        """
        Infer patch grid size (height, width) from input tensor and patch size.
        """
        if x.dim() != 4:
            return None

        patch_size = self.patch_size or getattr(self.model.config, 'patch_size', None)
        ...
        if img_height % patch_h != 0 or img_width % patch_w != 0:
            return None

        num_patches_height = img_height // patch_h
        num_patches_width = img_width // patch_w

        if patch_tokens.size(1) != num_patches_height * num_patches_width:
            return None

        return num_patches_height, num_patches_width

    def _split_tokens(self, tokens: torch.Tensor, x: torch.Tensor) -> Dict[str, Any]:
        """
        Split tokens into CLS, register, and patch tokens.
        """
        num_register_tokens = self.num_register_tokens or 0
        cls_token = tokens[:, 0, :]

        if num_register_tokens > 0:
            register_tokens = tokens[:, 1:1 + num_register_tokens, :]
        else:
            register_tokens = tokens.new_zeros((tokens.size(0), 0, tokens.size(2)))

        patch_tokens_flat = tokens[:, 1 + num_register_tokens:, :]
        patch_grid = self._infer_patch_grid(x, patch_tokens_flat)
        patch_tokens = None
        if patch_grid is not None:
            patch_tokens = patch_tokens_flat.unflatten(1, patch_grid)
    ...
    def forward(self, x: torch.Tensor, return_tokens: bool = False):
        """
        Forward pass through the DINOv3 encoder.

        Args:
            x (torch.Tensor): Input images of shape (batch, 3, height, width).
            return_tokens (bool): If True, return token splits with features.

        Returns:
            torch.Tensor or Tuple[torch.Tensor, Dict[str, Any]]:
                Encoded image features, optionally with token splits.
        """
        if x.dim() != 4:
            raise ValueError(
        ...
        token_info = self._split_tokens(tokens, x)
        cls_token = token_info['cls_token']
        register_tokens = token_info['register_tokens']
        patch_tokens_flat = token_info['patch_tokens_flat']

        if self.pooling_type == 'cls':
            features = cls_token
        elif self.pooling_type == 'register':
            if register_tokens.numel() > 0:
                features = register_tokens.mean(dim=1)
            else:
                features = cls_token
        elif self.pooling_type == 'mean':
            if patch_tokens_flat.numel() > 0:
                features = patch_tokens_flat.mean(dim=1)
            else:
                features = tokens.mean(dim=1)
        else:
            if patch_tokens_flat.numel() > 0:
                features = patch_tokens_flat.max(dim=1).values
            else:
    ...
    def from_checkpoint(
        ...
        """
        Create a DINOv3Encoder instance from a checkpoint file.
        """
        if not Path(path).exists():
            raise FileNotFoundError(f"Checkpoint not found: {path}")
